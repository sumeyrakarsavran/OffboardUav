...
def alcal():
	...
	if (cnt.level.pose.position.z +0.25)> altitude:
...
"""
    ...
    while not rospy.is_shutdown():
        cnt.updateSp()
        cnt.local_position_publisher.publish(cnt.level)
        rate.sleep()
        print("Suanki Yukseklik",altitude)
        if (cnt.level.pose.position.z +0.25)> altitude:
            print("ALCALDIGI DEGER=",altitude)
...
def main():
    ...
    alt_sub = rospy.Subscriber('mavros/altitude', Altitude, altitude_callback)
